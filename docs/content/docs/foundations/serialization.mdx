...
Third-party packages can export classes with custom serialization, `"use step"` functions, and `"use workflow"` functions for consumers to use. When you install such a package, the Workflow DevKit compiler must be able to process the package's code during the build — otherwise, its workflow constructs will not be transformed and will silently fail at runtime.

<Callout type="warn">
If a third-party package exports workflow constructs (`"use step"`, `"use workflow"`, or serialization classes), it must **not** be listed in your framework's package externalization config (e.g. `serverExternalPackages` in Next.js). Externalizing the package prevents the workflow compiler from processing it, which means:
- Step functions won't be registered for durable execution
- Workflow functions won't be assigned workflow IDs
- Serializable classes won't be registered for cross-boundary serialization

The build may appear to succeed, but workflow features from the package will not work at runtime.
</Callout>

If you see a build warning like:

```
⚠ Warning: "some-package" is listed in serverExternalPackages but contains
  workflow code. This code will not be transformed by the workflow compiler,
  which will cause runtime failures.
```

Remove the package from `serverExternalPackages` to fix this.
